...
**能力限制优先级（全包唯一口径）**：execution 实时 provider/adapter/version schema > 已选目标模型与适配器能力卡（用户指定或项目默认）> 用户明确暂不选型时的通用保守规划。第 0 步按配置引导继承目标模型与单次时长，已有值不重复问；两者均缺失且无相反项目约束时，普通规划才建议 **Dreamina / `seedance2.0fast_vip` / 15 秒一组**，不阻塞规划、不新增弹窗。明确单次 30 秒但未选型时依据模型卡给兼容建议，不能用普通默认覆盖目标。按实际选型加载命中卡；采用 Dreamina 执行 Seedance 时，`planning_only` 也加载 Seedance 与 Dreamina 两卡并注入景川；正式执行仍须能力、登录、配额与预算预检，默认选型不等于付费授权。
