...
**能力限制优先级（全包唯一口径）**：execution 实时 provider/adapter/version schema > 已选目标模型与适配器能力卡（用户指定或项目默认）> 用户明确暂不选型时的通用保守规划。本次明确要求优先于已批准项目设置，二者均无对应值时才用相容默认。视频分辨率未指定时默认 **720p**。Dreamina **未指定模型**时，按单次 Clip 请求时长选：**4–15 秒用 `seedance2.0fast_vip`；>15–30 秒用 `seedance2.5`**。模型与单次时长都未给时仍可建议 `seedance2.0fast_vip` / 15 秒一组；单集总长不用于代选单次模型。显式模型不兼容或时长不受支持时报告差异，不静默换模型、改时长或拆分；显式且兼容的模型/分辨率继续保留。按实际选型加载命中卡；采用 Dreamina 执行 Seedance 时，`planning_only` 也加载 Seedance 与 Dreamina 两卡并注入景川；正式执行仍须能力、登录、配额与预算预检，默认选型不等于付费授权。
...
| 触发条件 | 唯一正文格式 | 时长与调用语义 |
|---|---|---|
| 用户明确要求单次 30 秒/一个 30 秒 Clip/30 秒直出，或项目已启用专项 | 本节 2B 八段式 | 单次固定 `00:00–00:30`；一 Clip 一 Prompt Unit，不与四区块双交；不要求再次点名专项 |
| 未命中上述目标，或其他时长/普通任务 | 本节 2A 通用四区块 | 沿用项目锁与实际模型/模式时长；原 15 秒、10 秒、8 秒等分支不变 |

仅参考视频长 30 秒、示例 Prompt 为英文或案例使用某模型，不据此启用专项或锁定目标语言/模型；单集总长也不等于一次调用时长。目标明确却执行渠道不支持时报告差异，不静默换格式或调用单位。
...
中文项目使用同一字段职责并将字段名、正文与对白切换为中文；标题采用上表中文列。Block 1 可消费**单张 16:9 四区参考总表**：独立角色参考、实际关键道具、按需精选构图、纯场景；不强制关键帧。总表是普通参考，不冒充首帧；Panel 编号、素材顺序和绑定职责必须来自已可达文件。单图只是优先复用形式，不是强制：已有多图角色、首帧、环境或故事板参考可直接沿用，只要每项职责、优先级与冲突保留项写清。参考资产画幅与成片画幅分别继承，不能因总表为 16:9 就覆盖项目成片画幅。
...
**30 秒直出专项**：固定一 Clip 覆盖 `00:00–00:30`，可从三个镜头起排，但镜数仍服从已有一镜到底、项目明确镜数、完整表演与实际切点。英文首轮容量按 dim-audio 的 40–55 发音词、130–150 WPM 与气口/反应共同核算，40 不是补话下限；中文仍按 3–3.5 发音字/秒首估。容量超出必须重排或回剧情层，不在专项里静默延长、拆成两个 15 秒 Prompt Unit 或加速。
...
- [ ] 命中维度的契约要素均按观看重点与各镜职责回填到叙事目的、实际画面/声音或参考职责，不只列方法名；不承担该功能的镜头未被要求齐套，无职责缺项
...
- [ ] 已选模型与模式匹配时长/多镜/声音能力；默认建议满足 §1 前置条件且不覆盖目标锁，命中模型与 provider/adapter 卡实际注入
