...
> 解决的问题：音效、剧情内音乐与背景配乐谁管、怎么写进提示词、全剧配乐怎么规划。
...
2. **先分剧情内音乐与背景配乐**：角色歌唱、演奏、戏曲、舞台/仪式音乐或设备内播放属于剧情内声源，按剧本写表演者/声源、声部/乐器、进入/接续/中断及戏剧结果；“无 BGM”不删除它。背景 cue 才使用进入点（可见转折）+ 情绪方向 + 能量 + 退出（停止/解决/淡出/切静默）四要素。
3. **安全时长（music_window）**：**不默认全片 BGM**——背景 cue 只在改变情绪方向时加入，窗口不超过实际 Clip。剧情内表演按实际乐句/参考音轨与画面时间轴连续推进，不强制每个 Clip 完整结束乐句；跨独立生成只有实际音轨或自然乐句边界才可承诺连续，母题法/槽位法只保证主题身份，不冒充同一录音无缝续接。
...
5. Exclude 清单按该母版实例的实际审美取舍填写；例如需要克制弦乐母版时可排除 `piano/acoustic guitar/vocals/choir/drum kit/trailer hits/heroic melody/pop structure`，该例不升格为全项目或剧情内演奏的乐器禁令
...
| 缺少实际连续音轨或有效接续依据，却声称跨独立生成的旋律/录音无缝连续 | 有实际连续音轨时按其连续时间轴分配视觉分段；只有自然乐句边界时按接续/中断规划，不称同一录音无缝；二者皆无才改母题法（只保主题身份）或接受 clip-local，不写「无缝循环」 |
| “无 BGM”删掉剧情内歌唱/演奏 | 恢复剧本声源、声部/乐器与实际进度；只关闭额外背景 cue |
| 强迫每个 Clip 把剧情内乐句唱完/收尾 | 按实际音轨或自然乐句边界分配进度，保留接续/中断状态；跨独立 Clip 没有音轨时不伪称无缝 |
| 用母音色代替词曲/节奏参考 | 母音色只管声音身份；旋律、歌词、节拍另绑素材或明确标未锁设计建议 |
...
1. **供应商级权利矩阵（每首/每次生成必填）**：`provider / plan / generated_at（或 downloaded_at）/ terms_version（或 license_url）/ commercial_use / attribution / post_subscription_use / evidence_saved`。未知写 `not_verified`，不得凭「AI 生成」或「royalty-free」自动判可商用；核验只记录当时证据与待办，不作效果保证或超出许可证文本的权利保证。
