...
2. **先分剧情内音乐与背景配乐**：角色歌唱、演奏、戏曲、舞台/仪式音乐或设备内播放属于剧情内声源，按剧本写表演者/声源、声部/乐器、进入/接续/中断及戏剧结果；“无 BGM”不删除它。清唱、伴奏被切断或设备故障时明确在场人物实际听见的声层，不因高潮到来就恢复鼓组/贝斯；剧外配乐如获准，只在不会混淆清唱实力证据的区间进入，评审不能回应剧外 cue。背景 cue 才使用进入点（可见转折）+ 情绪方向 + 能量 + 退出（停止/解决/淡出/切静默）四要素。
