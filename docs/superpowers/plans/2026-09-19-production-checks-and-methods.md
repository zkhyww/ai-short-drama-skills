...
**Architecture:** 先在隔离工作区保存可发布的本机基线，再增加一个标准库检查器；Dreamina 文件入口消费同一原文和检查结果，普通字符串入口保持兼容。方法仍归已有 Crew／Studio 主责段落，不另建路由平台或规则层。按更新设计 §7，只读看板可独立读取显式输入，本计划的解析／检查接口完成后再按需接入，不作为看板和图册的前置条件。本计划继续独立待审，不因本轮资产交付范围获准而自动启动。
