...
| `drama-studio` | 1.13.5 | 30 | 从剧本交接到资产、分镜、声音、剪辑与 QC；含 Dreamina 路由与 ffmpeg 合成脚本 |
...
- Seedance 2.0/2.5 默认使用原生音视频联合生成；外部 WAV/TTS 只在音色或口型失败时后备。时间线脚本会先标准化异构镜头，再用 ffmpeg 拼接并由 ffprobe 验证。
